...
/**
 * Possible text size options exposed to consumers.
 */
export type TextSize = "small" | "normal" | "large";

/**
 * Context shape exposed by `useTextSize`.
 */
interface TextSizeContextType {
  /** Current selected size */
  size: TextSize;
  /** Setter to update the size and persist to localStorage */
...
/**
 * Provider that manages a persistent text-size preference.
 *
 * Stores the selected size in `localStorage` under the `text-size` key and
 * applies a root font-size scale to the document element. Consumers can read
 * and update the value via `useTextSize()`.
 */
...
/**
 * Hook to access current text size and an updater.
 *
 * Throws when used outside of `TextSizeProvider` so consumers don't silently
 * fail when the provider is not mounted.
 */
