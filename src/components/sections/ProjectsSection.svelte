...
<section class="space-y-6 mb-12" id={sectionId}>
    {#if !noTitle}
        <div class="space-y-3 mb-2">
            ...
            {#if description}
                <p
                    class="text-muted-foreground max-w-2xl text-sm md:text-base leading-relaxed"
                >
                    {description}
                </p>
            {/if}
    ...
    <div class="space-y-8">
        {#each projects as project, index (project.slug)}
            <Card
                class="border-border/60 overflow-hidden group hover:border-primary/40 transition-[transform,box-shadow,border-color] duration-300 rounded-none hover:-translate-y-1 hover:shadow-lg motion-reduce:hover:translate-y-0 {index %
                    2 ===
                0
                    ? 'bg-card/90'
            ...
            >
                <CardHeader class="pb-3 border-b border-border/50">
                    ...
                    >
                        <div
                            class="flex flex-col sm:flex-row gap-4 min-w-0 flex-1 items-start"
                        >
                            <div
                            ...
                            <div class="min-w-0 overflow-hidden">
                                <CardTitle
                                    class="type-h4 flex flex-wrap items-center gap-2 break-words"
                                >
                                ...
                                <p
                                    class="mt-1 max-w-[65ch] text-sm leading-relaxed break-words text-muted-foreground"
                                >
                                    {project.description}
                                </p>
                            </div>
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            <Button
                                ...
                                class="h-8 font-mono text-xs">Details</Button
                            >
                            {#if project.github}
                                <Button
                                    variant="outline"
                ...
                <CardContent class="pt-4 grid md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        <div>
                            ...
                            <p
                                class="text-sm text-muted-foreground border-l-2 border-primary/20 pl-3"
                            >
                                {project.problem}
                            </p>
                        </div>
                    ...
                    <div class="space-y-4 flex flex-col justify-between">
                        <div>
                            ...
                            <div
                                class="bg-secondary/50 p-3 rounded text-sm font-mono text-primary border border-border"
                            >
                                &gt;
                                {project.outcome}
                            </div>
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-1.5">
                                {#each project.technologies as tech (tech)}
                                    <TechBadge
                                        variant="outline"
                                        class="uppercase border-primary/20 bg-primary/5 text-primary"
                                    >
                                        {tech}
                                    </TechBadge>
                                {/each}
    ...
    {#if !noLink}
        <div class="flex justify-center mt-4">
            <Button
                variant="ghost"
                href="/projects"
                class="font-mono text-muted-foreground hover:text-primary"
            >
                View All Archives &gt;
            </Button>
        </div>
