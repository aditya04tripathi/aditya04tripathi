    ...
    async function exec(c: CommandDefinition) {
        try {
            if (c.kind === 'nav') {
                try {
                    const { navigate } =
                        await import('astro:transitions/client');
        ...
        >
            ...
            >
                ...
                <input
                    ...
                    aria-activedescendant={visible[selected]
                        ? `cmd-${visible[selected].id}`
                        : undefined}
            ...
            >
                <span
                    ><kbd class="rounded border border-border px-1">↑</kbd><kbd
                        class="rounded border border-border px-1">↓</kbd
                    >
                    Move</span
                >
                <span
                    ><kbd class="rounded border border-border px-1">↵</kbd> Open</span
                >
                <span
                    ><kbd class="rounded border border-border px-1">Tab</kbd> Focus</span
                >
                <span
                    ><kbd class="rounded border border-border px-1">Esc</kbd> Close</span
                >
            </div>
        </div>
    </div>
...
<style>
    @media (prefers-reduced-motion: no-preference) {
        .command-palette-panel {
            animation: command-palette-in 180ms cubic-bezier(0.25, 1, 0.5, 1)
                both;
        }
    }
